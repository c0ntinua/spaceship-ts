function getMousePosition(canvas : HTMLCanvasElement, event : MouseEvent) {
    ...
    if (y <= rule_digit_height) {
        ...
        if (cells[get(target_col,target_row)] >= states) cells[get(target_col,target_row)] = 0;
        calculatePartialAutomata(target_row);
        plotAutomata();
    }

}
function userUpdateFilter(x : number, y : number) {
    let target_index = 0;
    if (x >= 0 && x <= canvas.width) {
        target_index = Math.floor((x - rule_right_adjustment)/rule_digit_width);
        filter[target_index]++;
        if (filter[target_index] >= states) filter[target_index] = 0;
    }
    plotRule();
